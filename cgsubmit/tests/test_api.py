...
class TestApi(unittest.TestCase):
    ...
    def test_get_all_battles(self, mock_requests):
        ...
        try:
            ...
            self.assertTrue(str(exception).startswith('Unable to retrieve last battles: '))
            pass
        else:
    ...
    def test_get_lost_games(self, mock_requests):
        ...
        mock_requests.post.return_value = mock_response
        api = Api('')
        self.assertEqual(api.get_lost_games([1,2,3]),  ['mocked result', 'mocked result', 'mocked result'])
